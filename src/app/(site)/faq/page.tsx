    ...
    <main className="flex-1 px-6 pb-20 pt-12 sm:pb-24 sm:pt-16">
      ...
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:gap-16">
        <div className="flex flex-col gap-4 lg:sticky lg:top-28 lg:self-start">
          <h1 className="type-page text-text-primary">Questions, answered</h1>
        ...
        <dl className="flex flex-col divide-y divide-border-subtle border-y border-border-subtle">
          {FAQS.map((item) => (
            <div key={item.q} className="flex flex-col gap-2 py-6">
              <dt className="type-subhead text-text-primary">{item.q}</dt>
              <dd className="max-w-[62ch] leading-relaxed text-text-secondary">
                {item.a}
                {item.link && (
                  <>
                    {" "}
                    <Link href={item.link.href} className="font-medium text-text-accent underline underline-offset-4">
                      {item.link.label}
                    </Link>
                  </>
                )}
